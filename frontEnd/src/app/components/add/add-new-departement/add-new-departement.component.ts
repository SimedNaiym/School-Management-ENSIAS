...
export class AddNewDepartementComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.newDepartementFormGroup = this.fb.group({
      libelle_Dep: this.fb.control(null, [Validators.required]),
      Abrv_Dep: this.fb.control(null, [Validators.required])
    });
  }
  ...
  handleAddDepartement() {
    ...
    this.dpService.saveDepartment_(newDepart).subscribe({
      next: data => {
