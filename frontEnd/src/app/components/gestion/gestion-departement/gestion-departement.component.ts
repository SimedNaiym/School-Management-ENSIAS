...
export class GestionDepartmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.departmentService.searchDepartments_().subscribe(
      (response)=>{
  ...
  handleDeleteDepartment(department: Departement): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.departmentService.deleteDepartment_(department.id_Dep).subscribe();
         this.departments = this.departments.filter((d) => d.id_Dep !== department.id_Dep);
